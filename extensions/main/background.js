...
const DASHBOARD_URL = "http://localhost:3000"; // Ganti dengan URL dashboard Anda
  ...
  try {
    ...
    if (cookiesJson && isEncrypted(cookiesJson)) {
      try {
        ...
        // Jika gagal dekripsi, gunakan nilai asli (mungkin belum terenkripsi)
      }
    }
  ...
  (async () => {
    try {
      if (request.type === "CHECK_GUARD") {
        ...
        services = await fetchServices(userSession.access_token);
        // Simpan semua data sensitif terenkripsi
